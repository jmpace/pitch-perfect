...
export interface FrameworkPointMetrics {
  [key: string]: {
    min?: number;
    max?: number;
...
export interface FrameworkPointCriteria {
  description: string;
  metrics?: FrameworkPointMetrics;
}
...
export interface FrameworkPoint {
  ...
  evaluationCriteria: {
    excellent: FrameworkPointCriteria; // 8-10 score criteria
    satisfactory: FrameworkPointCriteria; // 5-7 score criteria
    needsImprovement: FrameworkPointCriteria; // 1-4 score criteria
  };
  targetMetrics?: {
    optimal?: string;
    minimum?: string;
...
export interface ComprehensiveFrameworkScore {
  sessionId: string;
...
export const FRAMEWORK_CATEGORIES: Record<string, FrameworkCategory> = {
  speech: {
    id: 'speech',
...
export const FRAMEWORK_POINTS: FrameworkPoint[] = [
  // Speech Mechanics (5 Points)
  {
...
export function getFrameworkPointById(id: string): FrameworkPoint | undefined {
  return FRAMEWORK_POINTS.find(point => point.id === id);
}

export function getFrameworkPointsByCategory(category: string): FrameworkPoint[] {
  return FRAMEWORK_POINTS.filter(point => point.category === category);
}

export function calculateCategoryScore(scores: FrameworkScore[], category: string): number {
  const categoryPoints = getFrameworkPointsByCategory(category);
  const categoryScores = scores.filter(score => 
    categoryPoints.some(point => point.id === score.pointId)
  );
...
export function calculateOverallScore(scores: FrameworkScore[]): number {
  ...
  const weightedScore = (
    categoryScores.speech * (FRAMEWORK_CATEGORIES.speech.weight / 100) +
    categoryScores.content * (FRAMEWORK_CATEGORIES.content.weight / 100) +
    categoryScores.visual * (FRAMEWORK_CATEGORIES.visual.weight / 100) +
    categoryScores.overall * (FRAMEWORK_CATEGORIES.overall.weight / 100)
  );

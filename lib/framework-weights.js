...
exports.CATEGORY_CONSISTENCY = exports.WEIGHT_VALIDATION = exports.INDIVIDUAL_POINT_WEIGHTS = exports.WEIGHT_RATIONALES = void 0;
exports.validateFrameworkWeights = validateFrameworkWeights;
exports.getWeightRationale = getWeightRationale;
exports.getIndividualPointWeight = getIndividualPointWeight;
exports.validateCategoryPointConsistency = validateCategoryPointConsistency;
exports.generateWeightSummary = generateWeightSummary;
const scoring_framework_1 = require("./scoring-framework");
// Weight Assignment Rationale and Justification
exports.WEIGHT_RATIONALES = [
    {
        categoryId: 'content',
        weight: 40,
...
exports.INDIVIDUAL_POINT_WEIGHTS = {
 ...
 */
function validateFrameworkWeights() {
    const issues = [];
    const recommendations = [];
    // Check category weights sum to 100%
    const totalCategoryWeight = Object.values(scoring_framework_1.FRAMEWORK_CATEGORIES)
        .reduce((sum, category) => sum + category.weight, 0);
    // Check individual point weights sum to 100%
    const totalPointWeight = Object.values(exports.INDIVIDUAL_POINT_WEIGHTS)
    ...
    if (scoring_framework_1.FRAMEWORK_CATEGORIES.content.weight < scoring_framework_1.FRAMEWORK_CATEGORIES.speech.weight) {
    ...
    if (scoring_framework_1.FRAMEWORK_CATEGORIES.visual.weight > scoring_framework_1.FRAMEWORK_CATEGORIES.speech.weight) {
 ...
 */
function getWeightRationale(categoryId) {
    return exports.WEIGHT_RATIONALES.find(rationale => rationale.categoryId === categoryId);
}
/**
 * Calculates effective weight for an individual framework point
 */
function getIndividualPointWeight(pointId) {
    return exports.INDIVIDUAL_POINT_WEIGHTS[pointId] || 0;
}
/**
 * Validates that individual point weights match their category weights
 */
function validateCategoryPointConsistency() {
    const results = {};
    Object.values(scoring_framework_1.FRAMEWORK_CATEGORIES).forEach(category => {
        const categoryPointIds = Object.keys(exports.INDIVIDUAL_POINT_WEIGHTS)
...
exports.WEIGHT_VALIDATION = validateFrameworkWeights();
exports.CATEGORY_CONSISTENCY = validateCategoryPointConsistency();
